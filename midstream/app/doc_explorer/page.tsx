...
export default function DocExplorer() {
    ...
    const handleDelete = (id: string) => async () => {
        const response = await fetch('/api/document_delete', {
            method: 'POST',
            headers: {
                                        ...
                                        <tbody>
                                            {documentsShown.map((doc, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className="py-3 px-5 border-b border-blue-gray-50">
                                                        <button onClick={ handleDelete(doc.id) } aria-expanded="false" aria-haspopup="menu" id=":r2:" className="relative middle none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-700 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30" type="button">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6">

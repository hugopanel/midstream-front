export default function Tasks() {
  ...
  //-------Enums a prédéfinir, voir avec tt le monde---------------------------------------------------------------------------------
  ...
  //---------------Html, et css à voir avec Lou cor doit étre définit-----------------------------------------------------------------------------
  
  // Liste des couleures prédéfinies pour les tags
  ...
  const taskTypeIcon: Record<taskType, JSX.Element> = {
  ...
  //-----------Data (pas besoin de s'en occupé car sera pull depuis la database)---------------------------------------------------------------------------------
    ...
    tasks.map((task) => (
        <div key={task.id} className="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md">
          ...
          <div className="border-t border-blue-gray-50 p-4">
            {task.tags.map((tag) => (
                <span key={tag.name} className={"inline-flex items-center rounded-md px-4 py-2 text-sm font-medium ring-1 ring-inset mr-[10px] " + colorClasses[tag.color]}> {tag.name} </span>
              ))}
          </div>
        </div>
      ))

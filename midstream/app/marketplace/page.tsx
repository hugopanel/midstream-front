...
import React, { useState, useEffect, use, ReactNode, FormEventHandler, ChangeEventHandler } from "react";
...
export default function Marketplace() {
	...
	useEffect(() => {
		setFilteredModules(modules.filter((modules) => modules.name.toLowerCase().includes(submitSearch.toLowerCase())))
	}, [submitSearch, modules])
								...
								<div className={`p-4 flex flex-col items-end text-right bg-gradient-to-r from-white from-70% to-${ModuleTemplates[module.id] ? ModuleTemplates[module.id].colorTo : ModuleTemplates[0].colorTo} bg-clip-border rounded-xl rounded-b-none`}>
									<button onClick={() => handleCheck(module.id)} >
										{module.checked ?
											...
											:
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 text-blue-gray-900 mb-5">

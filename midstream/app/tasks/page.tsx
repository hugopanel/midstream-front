...
const App: React.FC = () => {
  ...
  const handleDrop = (
    ...
    if (dragTemp && dropRef.current) {
      ...
      if (
        dropRef.current === dp2Ref.current ||
        dropRef.current === dp3Ref.current
      ) {
        dropRef.current.querySelectorAll(".drag").forEach((element) => {
          console.log(dropRef.current.id);
          console.log((element as HTMLElement).id);
  ...
  const getTasks = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to get the members of the team"
        );
      }
      ...
      for (let type in data.types) {
        var option = {
          ...
          label: data.types[type],
          checked: false,
        };
        optiontagsfilter.push(option);
      }
      optiontagsfilter.push({
        value: "Mine",
        label: "My tasks",
        checked: false,
      });
      ...
      for (let priority in data.priorities) {
        var option = {
          value: data.priorities[priority],
          label: data.priorities[priority],
          checked: false,
        };
      ...
      filterstoadd.push({
        id: "priority",
        name: "Urgency",
        options: optionprioritiesfilter,
      });
      filterstoadd.push({
        id: "dates",
        name: "Due Date",
  ...
  const saveTasks = async () => {
    ...
    for (let task in tasksToSave) {
      tasksToSave[task].beginningDate = convertToISOFormat(
        tasksToSave[task].beginningDate
      );
      ...
      for (let option in filtersDb[section].options) {
        ...
        if (optionObj.checked) {
          ...
          if (sectionObj.id === "dates") {
            ...
            if (value === "today") {
              filteredTasks = filteredTasks.filter(
                (task) => task.endDate === formatDateTime(new Date())
              );
            }
            if (value === "week") {
              filteredTasks = filteredTasks.filter(
                (task) =>
                  parse(task.endDate, "dd/MM/yyyy", new Date()) >=
      ...
      <div className="p-4 xl:ml-80">
        <nav className="block w-full max-w-full bg-transparent text-white shadow-none rounded-xl transition-all px-0 py-1">
          <div className="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
                                ...
                                <a
                                  ...
                                  className={classNames(
                                    option.current
                                      ...
                                      : "text-gray-500",
                                    active
                                      ? "bg-gray-100 focus:outline-none text-white"
                                      : "",
                    ...
                    <div className="flow-root">
                      <Popover.Group className="-mx-4 flex items-center divide-x divide-gray-200 focus:outline-none">
                        {FiltersDb.map((section, sectionIdx) => (
                          <Popover
                            key={section.name}
                            className="relative inline-block px-4 text-left  capitalize focus:outline-none"
                          >
            ...
            <div className="flex items-center">
              <button
                className="relative middle none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-700 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30 grid xl:hidden"
                type="button"
              >
              ...
              <a href="./profile_page">
                <button
                  className="middle none font-sans font-bold center uppercase transition-all disabled:opacity-90 disabled:shadow-none disabled:pointer-events-none text-xs py-3 rounded-lg text-gray-700 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30 hidden items-center gap-1 px-4 xl:flex"
                  type="button"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <a href="./profile_page">
                <button
                  className="relative middle none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-700 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30"
                  type="button"
                >
                  <span className="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">
                    <svg
        ...
        <div className="mt-7 flex relative items-start">
          ...
          >
            ...
            >
              {tasksToDo.map((task, index) => (
                <div
                  key={task.id}
                  id={`${task.id}`}
                  className={`drag w-full ${
                    task.priority === "high" ? "bg-blue-100" : "bg-blue-200"
                ...
                >
                  <div className="relative flex flex-col bg-clip-border rounded-xl bg-white/80 text-gray-700 shadow-md">
                    <div
                      className={`bg-clip-border mx-4 rounded-3xl overflow-hidden shadow-[${hexToRgb(
                        task.colour
                      )}] text-white shadow-lg absolute mt-2 grid h-13 w-13 place-items-center`}
                      style={{
                        backgroundColor: hexToRgb(task.colour),
                        boxShadow: `0px 10px 15px -3px ${hexToRgb(
                          task.colour
                        )}`,
                      }}
                    >
                    ...
                    <div className="p-4 text-right">
                      ...
                      <h4 className="block antialiased tracking-normal font-sans text-2xl font-semibold capitalize leading-snug text-blue-gray-900">
                        {task.title}
                      </h4>
                    </div>
                    <div className="border-t border-blue-gray-50 p-4 flex justify-end">
                      <span className="inline-flex items-center capitalize rounded-md bg-green-50 px-4 py-2 text-sm font-medium text-green-600 ring-1 ring-inset ring-green-500/10 mr-[10px]">
                        {task.typeOfTask}
                      </span>
            ...
            <button
              onClick={(e) => addTask("To do")}
              className="text-lg font-semibold mb-2 pt-2 mt-2 flex items-center bottom-0"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                ...
                stroke-width="1.5"
                stroke="currentColor"
                className="h-5 w-5 text-white font-semibold"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"

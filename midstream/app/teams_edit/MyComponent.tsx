...
export default function MyComponent() {
    ...
    const getMemberstoAdd = async (teamId:string) => {
        // setLoadingEmail('Loading...');
        // setErrorEmail('');
    ...
    useEffect(() => {
        const teamIdQuery = searchParams?.get('teamId')||"null";
        fetchRoles();
        console.log(teamIdQuery);
        if(teamIdQuery == "null" || teamIdQuery == "" || teamIdQuery == null) {
            console.log('create');
        }else{
    ...
    const handleRoleChange = (memberId:string, roleId:string) => {
      ...
      const handleRoleToAddChange = (memberId:string, roleId:string) => {
      ...
      const handleRoleDelete = (memberId:string, roleName:string, listType:string) => {
        setMembers((prevMembers) =>
          prevMembers.map((member:Member) =>
            member.memberId === memberId
              ? {
                  ...member,
                  [listType]: member.roles.filter((role:string) => role !== roleName),
                }
              : member
          )
        );
      ...
      const handleRoleToAddDelete = (memberId:string, roleName:string, listType:string) => {
                                        ...
                                        <tr key={member.memberId} className={index % 2 === 0 ? 'bg-blue-100/20' : 'bg-white'}>
                                            ...
                                            <div className="p-4 grid grid-flow-row-dense grid-cols-5 grid-rows-1 justify-items-stretch">
                                                ...
                                                {member.roles.map((role) => (
                                                    <span key={role} className={`inline-flex items-center rounded-md bg-${role === 'admin' ? 'pink' : 'indigo'}-50 px-4 py-2 text-sm font-medium text-${role === 'admin' ? 'pink' : 'indigo'}-600 ring-1 ring-inset ring-${role === 'admin' ? 'pink' : 'indigo'}-500/10 mr-[10px]`}>
                                                    {role}
                                                    <button onClick={() => handleRoleToAddDelete(member.memberId, role, 'roles')}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5 ml-2 -mr-1">
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
                                                        </svg>
                                                    </button>
